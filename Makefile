...
KUBE_EVENTER_LDFLAGS=-w -X kube-eventer/version.Version=$(VERSION) -X kube-eventer/version.GitCommit=$(GIT_COMMIT)

fmt:
    find . -type f -name "*.go" | grep -v "./vendor*" | xargs gofmt -s -w

build: clean
	GOARCH=$(ARCH) CGO_ENABLED=0 go build -ldflags "$(KUBE_EVENTER_LDFLAGS)" -o kube-eventer  kube-eventer
